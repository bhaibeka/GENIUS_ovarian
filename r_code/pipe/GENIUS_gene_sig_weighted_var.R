...
saveres <- "saveres_mostvar10_weighted"
...
var.genes.sub1 <-  order(apply(X=data, MARGIN=2, FUN = weight.var, weights = subtyp.prop$angiogenic), decreasing=TRUE)[1:ceiling(ncol(data) * propv)]

annot.sub1 <- annot[var.genes.sub1 , , drop=FALSE]
data.sub1 <- data[ ,var.genes.sub1, drop=FALSE]

var.genes.sub2 <-  order(apply(X=data, MARGIN=2, FUN = weight.var, weights = subtyp.prop$non.angiogenic), decreasing=TRUE)[1:ceiling(ncol(data) * propv)]
...
for(i in seq(from=1, to=ncol(resampling), by=1) ) {
   ...
   sampling.cindex <- apply(X=data.sub1[resampling[,i], , drop=FALSE], MARGIN=2, FUN=concordance.index, surv.time=survd[[1]][resampling[,i]], surv.event=survd[[2]][resampling[,i]], weights=as.numeric(subtyp.prop$angiogenic[ resampling[,i] ] ), outx=TRUE, method="noether", na.rm=TRUE)
...
full.cindex <- apply(X=data.sub1, MARGIN=2, FUN=concordance.index, surv.time=demo$t.os, surv.event=demo$e.os, weights=as.numeric(subtyp.prop$angiogenic), outx=TRUE, method="noether", na.rm=TRUE)
...
for(i in seq(from=1, to=ncol(resampling), by=1) ) {
   ...
   sampling.cindex <- apply(X=data.sub2[resampling[,i], , drop=FALSE], MARGIN=2, FUN=concordance.index, surv.time=survd[[1]][resampling[,i]],  surv.event=survd[[2]][resampling[,i]], weights=as.numeric(subtyp.prop$non.angiogenic[ resampling[,i] ] ), outx=TRUE,  method="noether", na.rm=TRUE)
...
full.cindex <- apply(X=data.sub2, MARGIN=2, FUN=concordance.index, surv.time=demo$t.os, surv.event=demo$e.os, weights=as.numeric(subtyp.prop$non.angiogenic), outx=TRUE, method="noether", na.rm=TRUE) 

...
for(i in 1:length(dataSets)) {
   ...
   if(!file.exists(sprintf("%s", "yoshihara_predict")) ) { 
      system(sprintf("mkdir %s", "yoshihara_predict")) 
   }
   ...
   save(list=c(  "risk.full", "risk.full.unscaled", "sig.new"), file=sprintf("yoshihara_predict/%s.RData", saveres) , compress=TRUE  )
}

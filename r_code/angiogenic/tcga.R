...
demo <- read.table("clinical_patient_public_OV.txt", sep = "\t", header = TRUE, stringsAsFactors=FALSE)
...
demo$grade <- sub("G4","4",demo$grade)
demo$grade <- ordered(demo$grade)
demo$debulking.stage <- NA


setwd("/common/projects/trisch/Ovarian_cancer/tcga2011/data")
...
save(list=c("data","demo", "annot"), compress=TRUE, file="tcga_all.RData")

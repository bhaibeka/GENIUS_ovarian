#USAGE: R CMD BATCH GENIUS_survival_predic.R '--args SAVERES_PREDICTION DATASET_1_FOLDER DATASET_2_FOLDER ...' forest_plot_all.R
...
pdf(sprintf("/common/projects/trisch/Ovarian_cancer/plots/forest_plot_%s.pdf", saveres), width=15, height=5)
...
#### sig1 #########
cindex_summary <- combine.est(cindex_sum_sig1[,1],cindex_sum_sig1[,4])

metaplot(mn=cindex_sum_sig1[,1], se=cindex_sum_sig1[,4],  nn=cindex_sum_sig1[,4]^-2, labels=dataSets,  summlabel="Summary", summn=cindex_summary$estimate ,  sumse=cindex_summary$se, sumnn=cindex_summary$se^-2 , xlim=c(0,1), boxsize = 0.5, zero = 0.5,   xlab="c-index", ylab="datasets",  col=meta.colors(box="royalblue",line="darkblue",zero="firebrick",summary="black"))
abline(v=c(0.6,0.7),lty=2, col="gray72")
title( main = "Subtype 1 / Angiogenic")

#### sig2 ########
cindex_summary <- combine.est(cindex_sum_sig2[,1],cindex_sum_sig2[,4])

metaplot(mn=cindex_sum_sig2[,1], se=cindex_sum_sig2[,4],  nn=cindex_sum_sig2[,4]^-2, labels=  dataSets,  summlabel="Summary", summn=cindex_summary$estimate ,  sumse=cindex_summary$se, sumnn=cindex_summary$se^-2 , xlim=c(0,1), boxsize = 0.5, zero = 0.5,   xlab="c-index", ylab="datasets",  col=meta.colors(box="royalblue",line="darkblue",zero="firebrick",summary="black"))
abline(v=c(0.6,0.7),lty=2, col="gray72")
title( main = "Subtype 2/ Non angiogenic")

#USAGE: R CMD BATCH GENIUS_survival_predic.R '--args CENSOR_TIME PATIENT.TYPE SAVERES_PREDICTION SAVERES_GENESIG DATASET_1_FOLDER DATASET_2_FOLDER ...' surv_plot.R
    ...
    for(i in 1:length(args)){
       ...
       if( i == 1) {
         censorTime <- as.numeric( args[[i]] )
          ...
          saveres <-  args[[i]]          #folder with prediction 
       } else if(i == 4) {
          saveresGeneSig <-  args[[i]]   # folder with gene signature
       } else {    
...
setwd(sprintf("/common/projects/trisch/Ovarian_cancer/tcga2011/%s",saveresGeneSig))
...
for(i  in 1:length(dataSets) ) {
   ...
   pdf(sprintf("/common/projects/trisch/Ovarian_cancer/plots/surv_subtypes_%s_%s.pdf", dataSets[i],saveres ), width=20, height=25)
...
abline( v=sig.size.s2)   
   ...
   lines(x=cluster.x, y=cluster.y2,type="l",lwd=2,col="red" )

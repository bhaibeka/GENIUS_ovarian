...
for (i in 1:nfold) {
   ...
   if(!file.exists(sprintf("%s", "cross_validation")) ) { 
      system(sprintf("mkdir %s", "cross_validation_stemlike")) 
   }
   ...
   pdf( sprintf("cross_validation_stemlike/fold_%i_class_plot_train.pdf", i) , width=12, height=8) 
   ...
   pdf(sprintf("cross_validation_stemlike/fold_%i_subtype_probability_density.pdf", i) , width=8, height=8)
   ...
   pdf( sprintf("cross_validation_stemlike/fold_%i_class_plot_test.pdf", i) , width=12, height=8) 
   ...
   pdf(sprintf("cross_validation_stemlike/fold_%i_subtype_probability_density_test.pdf", i) , width=8, height=8)
   plot(density( class.cluster$z[,1], na.rm=TRUE ), main ="")
   lines( density( emclassi$z[,1]) , col="blue" )  
   abline(v=c(0,1)) 
   title(main = dataSets[i])
   ...
   save( list =c("risk.s1.save", "risk.s2.save", "risk.full.save", "cindex.sig1.save", "cindex.sig2.save",  "cindex.all.save", "s.ix.save", "s.ix.sizes", "nfold"), compress=TRUE, file="cross_validation_stemlike/cross_validation_stemlike.RData" )
...
#####################################################
   ...
   save( list =c( "risk.full.nosub.save", "cindex.all.nosub.save", "s.ix.save", "s.ix.sizes", "nfold"),   compress=TRUE, file="cross_validation_stemlike/cross_validation_stemlike_nosubtypes.RData" )

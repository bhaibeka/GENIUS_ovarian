...
risk.score <- function(sig, data, annot,do.mapping) {
   ...
   if(do.mapping) {
      ...
      for(i in 1:nrow(sig) ) {
         ...
         if( sig[i,2] %in% annot[,2] ) { 

            found.probs <- rownames( annot[ annot[,2] %in% sig[i,2], ] ) 
            ...
            data.sig <- cbind(data.sig, temp.data )
            
         }     
      }
            
      map.probes <- intersect(sig[,2], annot[,2])  
      sig.new <- sig[ sig[, 2] %in% map.probes, ]
...
setwd("/common/projects/trisch/Ovarian_cancer/tcga2011/genesig_stemlike_5mostvar_weighted_0613_all")
...
for(i  in 1:length(dataSets) ) {
  ...
  # load(sprintf("%s_analyze.RData", substring(dataSets[i], 1 , (nchar(dataSets[i])-4) )))
   ...
   for(j in 1:sig.size ) {
      ...
      if(j == 1 ){
         save( list =c("cindex", "hazard", "tdrr", "cindex.sig1", "cindex.sig2",  "hazard.sig1", "hazard.sig2", "tdrr.sig1", "tdrr.sig2", "sig.s2", "sig.s1","risk.full", "risk.s1", "risk.s2", "risk.s1.unscaled", "risk.s2.unscaled"), compress=TRUE, file=sprintf("%s/genius/genius_stemlike_%s_%s_%s.RData", saveres,  dataSets[i], "optimum", trainSet ))	 
      } else {

         save( list =c("cindex", "hazard", "tdrr", "cindex.sig1", "cindex.sig2",  "hazard.sig1", "hazard.sig2", "tdrr.sig1", "tdrr.sig2", "sig.s2", "sig.s1","risk.full", "risk.s1", "risk.s2", "risk.s1.unscaled", "risk.s2.unscaled"), compress=TRUE, file=sprintf("%s/genius/genius_stemlike_%s_%s_%s_%s.RData", saveres,  dataSets[i],sig.size1,sig.size2,trainSet ))	
      }
   }
}

...
saveres <- "saveres_mostvar_weighted"
...
sig.size.s1.k <- as.numeric(substring( names(which.max(sizeStab.s1$kuncheva)),6) )
sig.size.s1.d <- as.numeric(substring( names(which.max(sizeStab.s1$davis)),6) )
...
sig.size.s2.k <- as.numeric(substring( names(which.max(sizeStab.s2$kuncheva)),6) )
sig.size.s2.d <- as.numeric(substring( names(which.max(sizeStab.s2$davis)),6) )
...
sig.probes.s2.full <- ranking.sel.full.s1[ , "probe"]

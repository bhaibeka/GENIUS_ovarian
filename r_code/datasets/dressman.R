...
demo <- read.csv(file="OVCclinicalinfo.csv",head=TRUE,sep=",") 
...
demo$debulking.stage <-  sub('^Optimal\\s*.*', "O",demo$debulking.stage, perl = TRUE)	
demo$debulking.stage <- sub("^Suboptimal\\s*.*", "S",demo$debulking.stage, perl= TRUE)
